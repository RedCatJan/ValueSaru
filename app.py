...
@st.cache_resource
def initialiser_feuilles():
    spreadsheet = connexion_google()

    # Une seule récupération des métadonnées des onglets.
    worksheets = {
        ws.title: ws
    ...
    return (
        get_or_create("Scans"),
        get_or_create("Matchs"),
        get_or_create("Signals"),
    )


sheet_scans, sheet_matchs, sheet_signals = initialiser_feuilles()
...
def lire_scans_values():
    return sheet_scans.get_all_values()


@st.cache_data(ttl=60, show_spinner=False)
def lire_matchs_values():
    return sheet_matchs.get_all_values()


@st.cache_data(ttl=60, show_spinner=False)
def lire_signals_values():
    return sheet_signals.get_all_values()


def lire_values(ws):
    if ws.title == "Scans":
        return lire_scans_values()

    if ws.title == "Matchs":
        return lire_matchs_values()

    if ws.title == "Signals":
        return lire_signals_values()

    return ws.get_all_values()


def vider_cache_sheet(ws):
    if ws.title == "Scans":
        lire_scans_values.clear()

    elif ws.title == "Matchs":
        lire_matchs_values.clear()

    elif ws.title == "Signals":
        lire_signals_values.clear()
...
def read_records(ws):
    return values_to_records(
        lire_values(ws)
    )


def ensure_headers(ws, required_headers):
    values = lire_values(ws)
...
def now_paris():
    return now_utc().astimezone(PARIS_TZ)
...
def format_paris(dt):
    return dt.astimezone(PARIS_TZ).strftime(
        "%d/%m/%Y %H:%M:%S"
    )
...
def fetch_events_for_league(league_name, sport_key):
    url = (
...
def build_planning(selected_leagues):
    rows = []
    errors = []
    current_utc = now_utc()

    for league_name in selected_leagues:
        result = fetch_events_for_league(
            league_name,
            SPORTS[league_name],
        )
        ...
        for event in result["events"]:
            ...
            try:
                match_utc = parse_utc(commence_time)
            except Exception:
                continue

            minutes_before = (
                match_utc - current_utc
            ).total_seconds() / 60

            if minutes_before <= 0:
                continue

            match_paris = match_utc.astimezone(PARIS_TZ)
            ...
            rows.append({
                ...
                "match_dt_paris": match_paris,
                "kickoff": match_paris.strftime("%H:%M"),
...
def quality_check(values):
    ...
    if len(clean) < MIN_BOOKMAKERS:
        return (
            "VERIFY",
            f"Seulement {len(clean)} bookmaker(s)",
        )

    if (
        len(clean) >= 2
        and clean[1] > 0
        and clean[0] / clean[1] > OUTLIER_RATIO_MAX
    ):
        return (
            "VERIFY",
            (
                f"Cote {clean[0]:.2f} très éloignée "
                f"de la 2e {clean[1]:.2f}"
            ),
        )
...
def analyser_match(match, league_name, scan_id):
    ...
    match_time = parse_utc(commence_time)
    ...
    # La stratégie officielle ne conserve que les CLOSING.
    if timing != "CLOSING":
        return []
    ...
    for bookmaker in match.get(
        ...
        for market in bookmaker.get(
            ...
            all_odds[home].append(c1)
            all_odds["Draw"].append(cx)
            all_odds[away].append(c2)
    ...
    # --------------------------------------------------------
    # V5 : moyenne des probabilités démargées
    # --------------------------------------------------------
    ...
    for outcome in [
        ...
        rows.append({
            "scan_id": scan_id,
            "timestamp_paris": timestamp_paris,
            "league": league_name,
            "event_id": event_id,
            "home": home,
            "away": away,
            "match_time_paris": format_match_paris(
                match_time
            ),
            "minutes_before_match": round(
                minutes_before,
...
def scanner_ligue(
    ...
    result["matches_api"] = len(matches)
...
def load_signals_records():
    return read_records(sheet_signals)
...
def make_signal_id(
    ...
    return (
        f"SIG-{str(event_id)[:8]}-"
        f"{safe_outcome[:12]}"
    )


def upsert_signals(closing_rows):
    existing = load_signals_records()
    ...
    for row in closing_rows:
        ...
        # Si un signal officiel existe déjà,
        # un nouveau scan CLOSING actualise seulement
        # la référence de closing / CLV.
        ...
        # Aucun nouveau paper bet si le signal
        # n'est pas officiel.
        if (
            row["official_signal"]
            != "YES"
        ...
        new_signal = {
            ...
            "away": row["away"],
            "match_time_paris": (
                row["match_time_paris"]
            ),
            "minutes_before_match": (
                row["minutes_before_match"]
            ),
            "outcome": row["outcome"],
            "fair_odd_historical": (
                row["fair_odd_historical"]
            ),
            "bet_odd": row["best_odd"],
            "bookmaker": row["bookmaker"],
            "edge_historical_pct": (
                row["edge_historical_pct"]
            ),
            "quality_status": (
                row["quality_status"]
            ),
        ...
        existing_by_key[key] = new_signal
        created += 1
...
def get_budget_stats():
    ...
    stats = {
        ...
        "usable_after_reserve": None,
    }

    if scans.empty:
        return stats

    if "credits_used" in scans.columns:
        scans["credits_used_num"] = (
            pd.to_numeric(
                scans["credits_used"],
                errors="coerce",
            )
        ...
        scans["credits_used_num"] = 0

    if "timestamp_paris" not in scans.columns:
        return stats

    parsed = pd.to_datetime(
        scans["timestamp_paris"],
        format="%d/%m/%Y %H:%M:%S",
        errors="coerce",
    )

    scans["_parsed"] = parsed
    ...
    today_mask = (
        scans["_parsed"].dt.date
        == today
    )

    month_mask = (
        (
            scans["_parsed"].dt.year
            == current_month[0]
        )
        & (
            scans["_parsed"].dt.month
            == current_month[1]
        )
    )

    stats["today_used"] = int(
        scans.loc[
            today_mask,
            "credits_used_num",
        ].sum()
    ...
    if active_days:
        stats["avg_per_active_day"] = (
            stats["month_used"]
            / active_days
        )

    if "credits_remaining" in scans.columns:
        ...
        if not remaining_series.empty:
            stats["remaining"] = int(
                remaining_series.iloc[-1]
            )

    if stats["remaining"] is not None:
        stats["usable_after_reserve"] = max(
            0,
            stats["remaining"]
            - CREDIT_RESERVE,
        )
...
tab_planning, tab_scan, tab_dashboard, tab_results, tab_history = st.tabs(
    [
        "📅 Planning",
        "🎯 Closing Scanner",
        "📈 Dashboard",
...
with tab_scan:
    ...
    planning_rows, _ = build_planning(
        list(SPORTS.keys())
    )
    ...
    if not open_signals:
        ...
        st.write(
            (
                ...
                f"vs {selected_signal['away']}**"
            )
        )

        c1, c2 = st.columns(2)

        home_score = c1.number_input(
            (
                f"Buts "
                f"{selected_signal['home']}"
            ),
